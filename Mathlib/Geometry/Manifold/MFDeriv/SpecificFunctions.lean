...
Lemmas about `HasMFDeriv(Within)At` and `mfderiv(Within)` are intentionally omitted:
they abuse the defeq between `E'` and `TangentSpace 𝓘(𝕜, E') (f z)` (similarly for `g',F',p',q'`).
In general this defeq is not canonical; in this case (the tangent space of a vector space) it is.
Yet, there is a better abstraction to work with, the `mvfderiv` defined in
`Mathlib.Geometry.Manifold.MFDeriv.NormedSpace`. Instead of lemmas about `mfderiv` abusing this
defeq, prefer corresponding lemmas about `mvfderiv`.
